...
ListingAttributesContainer = get_class(
    'dashboard.listings.listings_attributes', 'ListingAttributesContainer')
...
class AbstractListingClass(models.Model):
    """
    ...
    options = models.ManyToManyField(
        'listings.Option', blank=True, verbose_name=_("Options"))

    class Meta:
        abstract = True
        app_label = 'listings'
        ordering = ['name']
        verbose_name = _("Listing class")
        verbose_name_plural = _("Listing classes")
...
class AbstractListing(models.Model):
    ...
    is_public = models.BooleanField(
        ...
        default=True,
        db_index=True,
        help_text=_("Show this listing in search results and listings."))


    # Title is mandatory for canonical products but optional for child products
    title = models.CharField(pgettext_lazy('Listing title', 'Title'),
    ...
    #: None for child products, they inherit their parent's listings class
    product_class = models.ForeignKey(
        'listings.ListingClass',
        null=True,
        blank=True,
        on_delete=models.PROTECT,
        verbose_name=_('Listing type'), related_name="listings",
        help_text=_("Choose what type of listing this is"))
    attributes = models.ManyToManyField(
        'listings.ListingAttribute',
        through='ListingAttributeValue',
        verbose_name=_("Attributes"),
        help_text=_("A listing attribute is something that this listing may "
                    "have, such as a size, as specified by its class"))
    #: It's possible to have options listing class-wide, and per listing.
    product_options = models.ManyToManyField(
        'listings.Option', blank=True, verbose_name=_("Listing options"),
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.attr = SimpleLazyObject(lambda: ListingAttributesContainer(listing=self))
    ...
    def get_absolute_url(self):
        ...
        """
        return reverse('listings:detail',
    ...
    def _clean_standalone(self):
        ...
        """
        if not self.title:
            raise ValidationError(_("Your listing must have a title."))
        if not self.product_class:
            raise ValidationError(_("Your listing must have a listing class."))
        if self.parent_id:
            raise ValidationError(_("Only child listings can have a parent."))

    def _clean_child(self):
        """
        Validates a child listing
        """
        if not self.parent_id:
            raise ValidationError(_("A child listing needs a parent."))
        if self.parent_id and not self.parent.is_parent:
            raise ValidationError(
                _("You can only assign child listings to parent products."))
    ...
    def get_title(self):
        ...
        return title
    get_title.short_description = pgettext_lazy("Listing title", "Title")

    def get_meta_title(self):
        title = self.meta_title
        return title or self.get_title()
    get_meta_title.short_description = pgettext_lazy("Listing meta title", "Meta title")

    def get_meta_description(self):
        meta_description = self.meta_description
        return meta_description or striptags(self.description)
    get_meta_description.short_description = pgettext_lazy("Listing meta description", "Meta description")
    ...
    get_product_class.short_description = _("Listing class")
...
class AbstractListingAttribute(models.Model):
    """
    ...
    def save_value(self, product, value):   # noqa: C901 too complex
        ...
        except ListingAttributeValue.DoesNotExist:
            
            value_obj = ListingAttributeValue.objects.create(
    ...
    def _validate_option(self, value, valid_values=None):
        if not isinstance(value, get_model('listings', 'AttributeOption')):
            raise ValidationError(
...
class AbstractListingAttributeValue(models.Model):
    """
    The "through" model for the m2m relationship between :py:class:`Listing <.AbstractListing>` and
    :py:class:`ListingAttribute <.AbstractListingAttribute>`  This specifies the value of the attribute for
