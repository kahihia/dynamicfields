...
ListingSelect = get_class('dashboard.listings.widgets', 'ListingSelect')
...
class ListingClassSelectForm(forms.Form):
    """
    ...
    product_class = forms.ModelChoiceField(
        ...
        queryset=ListingClass.objects.all())
...
class ListingSearchForm(forms.Form):
    title = forms.CharField(
        max_length=255, required=False, label=_('Listing title'))
...
def _attr_entity_field(attribute):
    # Listing entities don't have out-of-the-box supported in the ListingForm.
...
class ListingForm(SEOFormMixin, forms.ModelForm):
    FIELD_FACTORIES = {
...
class ListingClassForm(forms.ModelForm):
...
class ListingAttributesForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if attr_type in [ListingAttribute.OPTION, ListingAttribute.MULTI_OPTION] and not option_group:
