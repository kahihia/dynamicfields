...
from dashboard.utils.generic import ObjectLookupView

(ListingForm,
 ListingClassSelectForm,
 ListingSearchForm,
 ListingClassForm,
 ...
 OptionForm) \
    = get_classes('dashboard.listings.forms',
                  ('ListingForm',
                   ...
                   'AttributeOptionGroupForm',
                   'OptionForm'))
(ListingAttributesFormSet,
 AttributeOptionFormSet) \
    = get_classes('dashboard.listings.formsets',
                   ('ListingAttributesFormSet',
                   'AttributeOptionFormSet'))
ListingTable, AttributeOptionGroupTable, OptionTable \
    = get_classes('dashboard.listings.tables',
                  ('ListingTable', 'AttributeOptionGroupTable', 'OptionTable'))
...
class ListingListView(SingleTableView):
    ...
    template_name = 'oscar/dashboard/listings/product_list.html'
    form_class = ListingSearchForm
    productclass_form_class = ListingClassSelectForm
    ...
    def get_description(self, form):
        if form.is_valid() and any(form.cleaned_data.values()):
            return _('Listing search results')
        return _('Listings')
    ...
    def get_queryset(self):
        ...
        """
        queryset = Listing.objects.browsable_dashboard().base_queryset()
...
class ListingCreateRedirectView(generic.RedirectView):
    permanent = False
    productclass_form_class = ListingClassSelectForm

    def get_product_create_url(self, product_class):
        """ Allow site to provide custom URL """
        return reverse('dashboard:listings-product-create',
    ...
    def get_invalid_product_class_url(self):
        ...
        return reverse('dashboard:listings-product-list')
...
class ListingCreateUpdateView(generic.UpdateView):
    """
    ...
    template_name = 'oscar/dashboard/listings/product_update.html'
    model = Listing
        ...
        if self.creating:
            ...
            if parent_pk is None:
                ...
                self.product_class = get_object_or_404(
                    ListingClass, slug=product_class_slug)
            else:
                self.parent = get_object_or_404(Listing, pk=parent_pk)
    ...
    def process_all_forms(self, form):
        ...
        # can't use commit=False because ListingForm does not support it
        if self.creating and form.is_valid():
            self.object = form.save()

       
        is_valid = form.is_valid() 
        cross_form_validation_result = self.clean(form)
    ...
    def get_success_url(self):
        ...
        msg = render_to_string(
            'oscar/dashboard/listings/messages/product_saved.html',
            {
                'product': self.object,
        ...
        if action == 'continue':
            url = reverse(
                'dashboard:listings-product', kwargs={"pk": self.object.id})
        else:
            url = reverse('dashboard:listings-product-list')
...
class ListingDeleteView( generic.DeleteView):
    ...
    Supports the permission-based dashboard.
    """
    template_name = 'oscar/dashboard/listings/product_delete.html'
    model = Listing
    context_object_name = 'product'
...
class ListingLookupView(ObjectLookupView):
    model = Listing
...
class ListingClassCreateUpdateView(generic.UpdateView):

    template_name = 'oscar/dashboard/listings/product_class_form.html'
    model = ListingClass
    form_class = ListingClassForm
    product_attributes_formset = ListingAttributesFormSet

    def process_all_forms(self, form):
        """
        This validates both the ListingClass form and the
        ListingClassAttributes formset at once
...
class ListingClassCreateView(ListingClassCreateUpdateView):
    ...
    def get_title(self):
        return _("Add a new Listing type")

    def get_success_url(self):
        messages.info(self.request, _("Listing type created successfully"))
        return reverse("dashboard:listings-class-list")


class ListingClassUpdateView(ListingClassCreateUpdateView):
    ...
    def get_success_url(self):
        messages.info(self.request, _("Listing type updated successfully"))
        return reverse("dashboard:listings-class-list")

    def get_object(self):
        product_class = get_object_or_404(ListingClass, pk=self.kwargs['pk'])
        return product_class


class ListingClassListView(generic.ListView):
    template_name = 'oscar/dashboard/listings/product_class_list.html'
    context_object_name = 'classes'
    model = ListingClass

    def get_context_data(self, *args, **kwargs):
        ctx = super().get_context_data(*args, **kwargs)
        ctx['title'] = _("Listing Types")
        return ctx


class ListingClassDeleteView(generic.DeleteView):
    template_name = 'oscar/dashboard/listings/product_class_delete.html'
    model = ListingClass
    form_class = ListingClassForm
    ...
    def get_success_url(self):
        messages.info(self.request, _("Listing type deleted successfully"))
        return reverse("dashboard:listings-class-list")


class AttributeOptionGroupCreateUpdateView(generic.UpdateView):

    template_name = 'oscar/dashboard/listings/attribute_option_group_form.html'
...
class AttributeOptionGroupCreateView(PopUpWindowCreateMixin, AttributeOptionGroupCreateUpdateView):
    ...
    def get_success_url(self):
        ...
        url = reverse("dashboard:listings-attribute-option-group-list")
        return self.get_url_with_querystring(url)
...
class AttributeOptionGroupListView(SingleTableView):
    ...
    template_name = 'oscar/dashboard/listings/attribute_option_group_list.html'
    model = AttributeOptionGroup
...
class AttributeOptionGroupDeleteView(PopUpWindowDeleteMixin, generic.DeleteView):
    ...
    template_name = 'oscar/dashboard/listings/attribute_option_group_delete.html'
    model = AttributeOptionGroup
    form_class = AttributeOptionGroupForm
...
class OptionListView(SingleTableView):
    ...
    template_name = 'oscar/dashboard/listings/option_list.html'
    model = Option
...
class OptionCreateUpdateView(generic.UpdateView):
    ...
    template_name = 'oscar/dashboard/listings/option_form.html'
    model = Option
...
class OptionCreateView(PopUpWindowCreateMixin, OptionCreateUpdateView):
    ...
    def get_success_url(self):
        ...
        return reverse("dashboard:listings-option-list")
...
class OptionDeleteView(PopUpWindowDeleteMixin, generic.DeleteView):
    ...
    template_name = 'oscar/dashboard/listings/option_delete.html'
    model = Option
